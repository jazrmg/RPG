...
public partial class Player3D : CharacterBody3D
{
	...
	public bool _isSwordEquipped = false;  // ✨ SWORD NOT EQUIPPED AT START - REQUIRE MANUAL EQUIP!
	...
	// ✨ PHASE 1: Mobile camera control from right joystick
	private void HandleMobileCameraControl()
	{
		var touchControls = GetTree().Root.FindChild("TouchControls", true, false) as TouchControls;
		if (touchControls == null) return;

		Vector2 cameraInput = touchControls.CameraInput;
		if (cameraInput == Vector2.Zero) return;

		float sensitivity = 0.015f;  // Slow, smooth rotation
		
		_cameraYaw += cameraInput.X * sensitivity;
		_cameraPitch -= cameraInput.Y * sensitivity;
		_cameraPitch = Mathf.Clamp(_cameraPitch, Mathf.DegToRad(-60), Mathf.DegToRad(30));
	}
